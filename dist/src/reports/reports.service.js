...
let ReportsService = class ReportsService {
    ...
    async generate(familyId, userId, dto) {
        await this.familyService.assertMember(familyId, userId);
        const from = new Date(dto.from + 'T00:00:00.000Z');
        const to = new Date(dto.to + 'T23:59:59.999Z');
        const [family, events, custodyEvents, expenses, messages, mediationSessions, healthRecords, activeMedications, allergies,] = await Promise.all([
        ...
        for (const ev of custodyEvents) {
            ...
            if (!custodyMap[childName])
                custodyMap[childName] = {};
